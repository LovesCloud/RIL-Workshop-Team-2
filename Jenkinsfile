node {
   ...
   stage('Prepare') {
      ...
      git branch: 'develop', url: 'https://github.com/LovesCloud/RIL-Workshop-Team-2.git' 
 ...
 /*  stage('Sonar') {
      withSonarQubeEnv('SonarQube') {
        sh "${scannerHome}/bin/sonar-scanner -e -Dsonar.projectName=RIL-Workshop-Team-2 -Dsonar.projectKey=RILWT2 -Dsonar.sources=src -Dsonar.java.binaries=target/"
      }*/
   ...
   stage('Docker-Build') {
      sh"""#!/bin/bash
         docker build . -t crud-mysql-vuejs-rilwt2:${BUILD_NUMBER}
      """
   }
   ...
   stage('Docker-Push') {
      ...
      withDockerRegistry(credentialsId: 'dockerhub') {
         sh"""#!/bin/bash
             docker tag crud-mysql-vuejs-rilwt2:${BUILD_NUMBER} lovescloud/crud-mysql-vuejs-rilwt2:${BUILD_NUMBER}
             docker push lovescloud/crud-mysql-vuejs-rilwt2:${BUILD_NUMBER}
          """
      }
   ...
   stage('Trigger-Deploy') {
      sh label: '', script: '''sed -i \'s/IMAGE/image: lovescloud\\/crud-mysql-vuejs-rilwt2:\'${BUILD_NUMBER}\'/\' docker-compose.yaml
'''
      sh"""#!/bin/bash
